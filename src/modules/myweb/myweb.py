...
class MyWeb:
    ...
    def browser(self, action, subaction = None, condition = None):
        ...
        Args:
            ...
            action (str): A ação que será realizada ['GET', 'QUIT', 'CLOSE', 'PAGE_SOURCE', 'COMMAND_EXECUTOR', 'PRINT', 'MAXIMIZE', 'EXECUTE_SCRIPT', 'SWITCH_TO', 'CURRENT_URL', 'CURRENT_WINDOW_HANDLE'].
            subaction (str / opcional): A subação que irá realizar ['DOWNLOAD', 'SAVE', 'WINDOW'].
        ...
        try:
            ...
            if self.action is not None:
                ...
                # CLOSE 
                elif self.action.lower() == 'close':
                    self.webdriver.close()

                # PAGE SOURCE
                elif self.action.lower() == 'page_source':
                    self.resultado = self.webdriver.page_source

                # command executor 
                elif self.action.lower() == 'command_executor':
                    command_executor = self.webdriver.command_executor
                ...
                # EXECUTE_SCRIPT 
                elif self.action.lower() == 'execute_script':
                    self.resultado = self.webdriver.execute_script(condition)

                # SWITCH_TO 
                elif self.action.lower() == 'switch_to':
                    self.resultado = self.webdriver.switch_to

                # CURRENT_URL 
                elif self.action.lower() == 'current_url':
                    self.resultado = self.webdriver.current_url

                # CURRENT_WINDOW_HANDLE
                elif self.action.lower() == 'current_window_handle':
                    self.resultado = self.webdriver.current_window_handle

                # ELSE 
                else:
                    self.status = False
                    mensagem = '"Action" especificado não cadastrado.'
                    print(mensagem)
                ...
                # DOWNLOAD                         
                elif self.subaction.lower() == 'download':
                ...
                # SAVE                         
                elif self.subaction.lower() == 'save':
                    self.webdriver.save_screenshot(condition)
                ...
                # WINDOW                      
                elif self.subaction.lower() == 'window':
                    #self.resultado = self.webdriver.switch_to.window(self.webdriver.window_handles[condition])
                ...
                # NEW WINDOW                      
                elif self.subaction.lower() == 'new_window':
                    self.resultado = self.webdriver.switch_to.new_window()
    ...
    # Buscar elementos
    def element(self, element, type, action, subaction = None, condition = None, subcondition = None):
        ...
        Args:
            ...
            subaction (str / opcional): A subação que irá realizar ['INDEX', 'CLICK', 'VALUE', 'TEXT', 'LIST', 'SEND_KEYS', 'SCREENSHOT', 'GET_ATTRIBUTE', 'EXECUTE_SCRIPT', 'IS_DISPLAYED', 'GET_VALUE'].
                obs.: Por padrão, será 'None'.
            condition (str, int / opcional): A condição para realizar a subação.
                obs.: Por padrão, será 'None'.
        """
        ...
        try:
            ...
            if self.type is not None:
                ...
                # Verifica se não é um 'element'
                if self.type.lower() == 'element':
                ...
                if self.resultado:
                    ...
                    if self.action is not None:
                        ...
                        # FIND                         
                        if self.action.lower() == 'find' or self.action.lower() == 'find_element':
                            self.resultado_elemento = self.resultado

                        # FIND ELEMENTS                         
                        elif self.action.lower() == 'find_elements':
                            self.resultado = self.webdriver.find_elements(self.type, element)
                            self.resultado_elemento = self.resultado
                        ...
                        # CLICK
                            ...
                            # Verifica se type não é 'element'
                            if self.type.lower()!= 'element':
                                status_click = WebDriverWait(self.webdriver, 10).until(EC.element_to_be_clickable((type, element)))
                        ...
                        # SCREENSHOT
                        elif self.subaction.lower() == 'screenshot':
                            self.resultado_elemento = self.resultado.screenshot(condition)

                        # GET_ATTIBUTE
                        elif self.subaction.lower() == 'get_attribute':
                            if isinstance(self.resultado, list):                       
                                self.resultado_elemento = []
                                for resultado_count in self.resultado:
                                    resultado = resultado_count.get_attribute(condition)
                                    self.resultado_elemento.append(resultado)
                            
                            else:
                                self.resultado_elemento = self.resultado.get_attribute(condition)

                        # EXECUTE_SCRIPT
                        elif self.subaction.lower() == 'execute_script':
                            mensagem = self.resultado
                            if isinstance(self.resultado, list):
                                self.resultado_elemento = []
                                for resultado_count in self.resultado:
                                    resultado = self.webdriver.execute_script(condition)
                                    self.resultado_elemento.append(resultado)
                            
                            else:
                                self.resultado_elemento = self.webdriver.execute_script(condition)
