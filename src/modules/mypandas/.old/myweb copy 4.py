                    ...
                    if self.action is not None:
                        ...
                        # GET_ATTIBUTE
                            ...
                            if isinstance(self.resultado, list): 
                                self.my_logger.log_info("--------------------")
                                self.my_logger.log_info(self.resultado)                         
                                self.resultado_elemento = []
                                self.my_logger.log_info(self.resultado_elemento)
                                self.my_logger.log_info("---------------------")
                                for resultado_count in self.resultado:
                                    self.my_logger.log_info(resultado_count)
                                    self.my_logger.log_info("---------------------")
                                    #resultado = resultado_count.find_element(By.TAG_NAME, 'a').get_attribute(condition)
                        ...
                        # EXECUTE_SCRIPT
                            ...
                            self.my_logger.log_info(f'teste 1: {mensagem}')
                            if isinstance(self.resultado, list):                          
                                self.resultado_elemento = []
                                for resultado_count in self.resultado:
                                    resultado = self.webdriver.execute_script(condition, resultado_count)
                                    self.resultado_elemento.append(resultado)
                                    mensagem = self.resultado_elemento
                                    self.my_logger.log_info(f'teste 2: {mensagem}')
                            
                            else:
                                self.resultado_elemento = self.webdriver.execute_script(condition, resultado_count)
