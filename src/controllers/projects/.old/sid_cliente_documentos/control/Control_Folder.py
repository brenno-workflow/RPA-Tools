...
class ControlFolder():
    ...
    def __init__(self, 
                 ...
                 param_true, param_control, param_control_sucesso, param_control_falha, param_control_erro,
                 folder_control, folder_sucesso, folder_falha, folder_erro, 
    ...
    def control_folder(self):
        ...
        # Variaveis
        param_subcontrol = [self.param_control_sucesso, self.param_control_falha, self.param_control_erro]
        folder_subcontrol = [self.folder_sucesso, self.folder_falha, self.folder_erro]
        ...
        try:
            ...
            if status_control['status']:
                ...
                # Criar pasta SID
                status_control_service = self.subcontrol.subcontrol(self.param_control, self.param_control_service, self.folder_service)
                
                if status_control_service['status']:

                    # Criar pasta Cliente
                    status_control_module = self.subcontrol.subcontrol(self.param_control_service, self.param_control_module, self.folder_module)
                    ...
                    if status_control_module['status']:
                        ...
                        for param_subcontrol_count, folder_subcontrol_count in zip(param_subcontrol, folder_subcontrol):
                            ...
                            if status_control_module['status']:
                                self.status = True
                                mensagem = f'Sucesso ao criar as pastas: "{self.folder_control}", "{self.folder_service}" e "{self.folder_module}"'
                                print(mensagem)
            ...
            self.status = False
            mensagem = f'Erro ao criar as pastas: "{self.folder_control}", "{self.folder_service}" e "{self.folder_module}"'
            print(self.erro)
            print(mensagem)
            print(aviso)
