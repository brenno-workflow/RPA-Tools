...
class Mail():
    ...
    def __init__(self, 
                 ...
                 param_true, param_mail_repositorio, param_mail_ecossistemanf_sucesso, param_mail_ecossistemanf_falha, param_mail_ecossistemanf_erro, param_control_fsist_sucesso, param_control_fsist_falha, param_control_fsist_erro,
        ...
        self.param_mail_ecossistemanf_sucesso = param_mail_ecossistemanf_sucesso
        self.param_mail_ecossistemanf_falha = param_mail_ecossistemanf_falha
        self.param_mail_ecossistemanf_erro = param_mail_ecossistemanf_erro
        self.param_control_fsist_sucesso = param_control_fsist_sucesso
        self.param_control_fsist_falha = param_control_fsist_falha
        self.param_control_fsist_erro = param_control_fsist_erro
    ...
    def select(self, type):
        ...
        try:
            ...
            if type.lower() == 'sucesso':
                ...
                # Path
                filter_path = [self.param_control_fsist_sucesso, self.param_true]

            elif type.lower() == 'falha':
                status_type = True

                # Path
                filter_path = [self.param_control_fsist_falha, self.param_true]

            elif type.lower() == 'erro':
                status_type = True

                # Path
                filter_path = [self.param_control_fsist_erro, self.param_true]
            ...
            if status_type:                
                ...
                if status_mysql_path['status']:
                    ...
                    # Listar path
                    resultado = status_mysql_path['resultado'][0][0]
                    self.status = True
                    mensagem = f'Sucesso fazer o select do path: {resultado}.'
                    print(mensagem)

                else:
                    self.status = False
                    mensagem = f'Falha ao fazer o select do path: {resultado}.'
                    print(mensagem)
    ...
    def mail(self, type):
        ...
        try:
            ...
            if type.lower() == 'sucesso':
                ...
                filter_mail_sucesso = [self.param_mail_ecossistemanf_sucesso, self.param_true]                
                ...
                # Mail
                filter_mail_repositorio = [self.param_mail_repositorio, self.param_true]
                filter_mail_falha = [self.param_mail_ecossistemanf_falha, self.param_true]
                ...
                # Mail
                filter_mail_repositorio = [self.param_mail_repositorio, self.param_true]
                filter_mail_erro = [self.param_mail_ecossistemanf_erro, self.param_true]
                ...
                if status_mysql_path['status']:
                    ...
                    if status_myfolder_files['status']:
                        ...
                        if status_select:
                            ...
                            if type.lower() == 'falha':
                                file.append(self.log_file_path)
                            ...
                            if status_mymail_send['status']:
                                ...
                                status_myfolder_clean = self.my_folder.folder('remove', 'file', path)

                                if status_myfolder_clean['status']:
                                    self.status = True
                                    mensagem = f'Sucesso ao limpar o path: {path}.'
                                    print(mensagem)

                                else:
                                    self.status = False
                                    mensagem = f'Falha ao limpar o path: {path}.'
                                    print(mensagem)
    ...
    def postman(self):
        ...
        """
        Ira enviar todos os e-mails disponiveis
        """

        # Variaveis
        type = ['sucesso', 'falha', 'erro']

        # TryCtach
        try:

            # Iniciar loop
            for type_count in type:

                # Listar as pastas para ver se precisa mandar e-mails
                status_select = self.select(type_count)

                if status_select['status']:

                    path = status_select['resultado']

                    status_myfoler_file_count = self.my_folder.folder('list', 'file', path)

                    if status_myfoler_file_count['status']:

                        file_list = status_myfoler_file_count['resultado']

                        if len(file_list) > 0:

                            status_mail = self.mail(type_count)
                            ...
                            self.status = True
                            mensagem = 'Não existem arquivos para encaminhar por e-mail na pasta de controle.'
                            print(mensagem)

                    else:
                        self.status = False
                        mensagem = 'Falha ao listar os arquivos da pasta de controle.'
                        print(mensagem)

                else:
                    self.status = False
                    mensagem = 'Falha ao fazer o select da pasta de controle.'
                    print(mensagem)
